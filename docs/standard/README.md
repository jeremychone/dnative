...
`dnative` is a DOM-first, Web Component-oriented utility library for building modern frontends with native browser primitives and TypeScript.
The browser and the DOM are the framework.

This standard follows both the public `dnative` API and the recurring patterns used in real `dnative` applications:
...
```ts
import { BaseHTMLElement, customElement, first, html, onEvent } from "dnative";
...
The awesomeapp frontend uses a very consistent `dnative` style:
