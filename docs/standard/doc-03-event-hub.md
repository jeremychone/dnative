# dnative Standard, Hub Events

## Overview

This document covers the `dnative` hub system:
...
`dnative` also supports dictionary-based hub binding.

```ts
import { bindHubEvents } from "dnative";
...
```ts
import { BaseHTMLElement, customElement, onHub } from "dnative";
