# dnative Standard, DOM Events

## Overview

This document covers DOM event binding in `dnative`:
...
With delegated binding, `dnative` adds `evt.selectTarget`.

```ts
import type { OnEvent } from "dnative";
...
`dnative` provides decorators for component methods.
...
```ts
import { BaseHTMLElement, OnEvent, customElement, html, onEvent } from "dnative";
...
```ts
import { BaseHTMLElement, customElement, onDoc } from "dnative";
...
```ts
import { BaseHTMLElement, customElement, onWin } from "dnative";
