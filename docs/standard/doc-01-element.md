# dnative Standard, Elements

## Overview

This document covers the standard element pattern for `dnative`:
...
In short, this document explains how `dnative` components are structured, rendered, and composed using native custom elements plus the standard `html`, `elem`, and `frag` builder helpers.
...
```ts
import { BaseHTMLElement, customElement, first, html } from "dnative";
...
The standard `dnative` element style is:
