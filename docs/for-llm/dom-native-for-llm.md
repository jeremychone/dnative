...
`dnative` is a small TypeScript utility library for building apps directly with the real DOM and native custom elements.
...
- `dnative`, core DOM, events, custom element base class, pub/sub, positioning, DX helpers.
- `dnative-dnd`, drag and drop helpers and FLIP animation utility.
- `dnui`, UI components built on top of `dnative`, not covered here in depth.
...
File: `dnative/src/ts-decorator-custom-element.ts`
...
File: `dnative/src/ts-decorator-on-event.ts`
...
File: `dnative/src/ts-decorator-on-hub.ts`
...
Extended event shape used by `dnative`:
...
File: `dnative/src/dom-builders.ts`
...
- `dnative-dnd/src/draggable.ts`
- `dnative-dnd/src/flip.ts`
...
````ts
import { BaseHTMLElement, customElement } from "dnative";
...
`dnative` is a thin, opinionated utility layer over native browser APIs.
