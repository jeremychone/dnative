...
import { customElement, first, on, OnEvent } from "dnative";
import { DragEventDetail, draggable } from "dnative/dnd";
...
function simpleDroppable(rootEl: HTMLElement) {
	...
	// Note: using dnative on(...) which wrapped rootEl.addEventListener and add selector and event namespacing (not used here)
...
const spec_dnd_basic: CodeDoc = {
	title: "dnd basic",
	jsPrefix: `
import { on, first } from 'dnative'
import { draggable } from 'dnative/dnd'
	`,
	groups: [
		{
			items: [
