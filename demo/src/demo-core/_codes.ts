...
class AnimPie extends BaseHTMLElement {
	...
	async refresh() {
		...
		// Note: easing method is optional, dnative does not provide any. here easeBounce is from d3-ease.
...
@customElement("simple-element-2") // dnative provide this ts decorator
...
function positionSimple(rootEl: HTMLElement) {
	...
	on(rootEl, 'pointerup', '.clickable', async (evt) => {
		...
		// NOTE: here we do some cleanup, but the problem with SpecView not fully resolved becase of dnative aliased
