...
- Do not change `dnative` and `draggable` package-level build systems.
...
- Keep build behavior for subprojects unchanged:
  - `dnative/package.json` scripts untouched
...
- Route fallback behavior:
  - Missing hash, malformed hash, or invalid family routes redirect to dnative default route.
- Tab behavior:
...
- Router wiring:
  - Use dnative hub scheme with `router-hub`.
...
- dnative family default spec:
  - first dnative spec in configured list.
...
- Default load with no hash redirects to dnative default route.
- Invalid routes redirect to dnative default route.
...
- Router and family-first navigation are implemented:
  ...
  - Canonical route format is active:
    - `#dnative/<spec>`
    - `#draggable/<spec>`
  - Invalid or missing routes normalize to dnative default.
...
- A post-implementation issue was reported in the integrated `dnui` demo flow:
  - The header still appeared to show the old external `dnui` tab target in the running demo.
  - The demo build emitted unresolved import warnings for `dnui` from `demo/src/demo-ui/index.ts`.
  - Rolldown treated `dnui` as an external dependency and guessed a browser global name.

- Root cause summary:
  - Demo integration for the UI family was only partially wired.
  - The demo source imported `dnui`, but the demo bundling config did not provide a matching local alias for that package.
  - The demo TypeScript config also did not include a path mapping for `dnui`, nor include `dnui/src` in the demo compilation scope.
  - The route validation config for the `dnui` family was incomplete because the UI family was missing from `VALID_SPECS_BY_FAMILY`.

- Fix summary implemented:
  - Added local demo build aliasing for `dnui` in:
    - `demo/rolldown.config.js`
    - `demo/tsconfig.json`
  ...
  - Completed route/spec validation for the UI family by adding `dnui` to `VALID_SPECS_BY_FAMILY` in `demo/src/infra/spec-config.ts`.

- Expected result after this fix:
  - `dnui` resolves to local source during the demo build.
...
- A remaining integration issue was still present after the prior `dnui` demo wiring:
  - The UI styles did not load correctly in the integrated demo bundle flow.
  - The `dnui/css/main.css` file still imported non-existent `.pcss` files:
    - `./base.pcss`
    - `./var-colors.pcss`
    - `./var-elevs.pcss`
    - `./d-base-input.pcss`
...
- Root cause summary:
  - The actual files in `dnui/css/` are `.css` files, not `.pcss` files.
  - Because `demo/css/main.css` imports `../../dnui/css/main.css`, the broken import chain prevented the integrated UI CSS from resolving cleanly in the demo build.

- Fix summary implemented:
  - Updated `dnui/css/main.css` to import the existing `.css` files instead of `.pcss`.

- Expected result after this fix:
  - The integrated `dnui` demo CSS resolves through the existing demo CSS bundle import chain.
