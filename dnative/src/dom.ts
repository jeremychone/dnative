...
export function getFirst(
	...
	if (el == null) throw new Error("dnative - getFirst - requires el to not be null");

	const l = selectors.length;
	if (l == 0 || l == 1) {
		const res = _first(el, selectors[0]);
		if (res == null) throw new Error("dnative - getFirst - element not found");
		return res;
	} else {
		const res: HTMLElement[] = [];
		for (const sel of selectors) {
			const iel = _first(el, sel);
			if (iel == null) throw new Error(`dnative - getFirst - element for selector '${sel}' not found`);
			res.push(iel);
		}
		return res;
	}
...
export function cherryChild(
	...
	if (el == null) {
		throw new Error(`dnative - scanChild - requires el to not be null`);
	}
	...
	// 	throw new Error("dnative - scanChildren - node has less children than requested names");
	// }
	...
	if (result!.length < tagNamesLength) {
		throw new Error("dnative - scanChildren - node has less match children than requested");
	}
...
export function append(
	...
	// NOTE: we assume HTML element as per dnative current approach.
	...
	if (position === "last" || position === "first" || position === "empty") {
		...
		if (!parentEl) {
			throw new Error(
				"dnative ERROR - The referenceElement " + refEl + " does not have a parentNode. Cannot insert " + position,
			);
		}
	}
