...
## **dnative**: A Native Web Component TypeScript Optimized Utility

`dnative` is a minimalistic utility (<7kb gzip) embracing the DOM Native Web Components model to build simple to big applications the DOM way.  **#LearnWhatMatters** **#SimpleScaleBetter**

**THE DOM IS THE FRAMEWORK**

[QUICK DEMO](https://demo.dnative.org/core/index.html)
...
```ts 
import {customElement, BaseHTMLElement, onEvent} from 'dnative';
...
```ts
// main.ts
import { BaseHTMLElement, onEvent } from 'dnative';
...
> **dnative** is designed to scale from the get-go and, therefore, fully embrace TypeScript types and makes a lightweight, expressive, and optional use of TypeScript decorators (JS decorator standard is still way off). However, all functionalities are available in pure JS as well.


## Full BaseHTMLElement lifecycle and typescript decorators

dnative [BaseHTMLElement](src/c-base.ts) is just a base class that extends DOM native custom element class `HTMLElement` and add some minimalistic but expressive lifecycle methods as well as few typescript decorators (or properties for pure JS) to allow safe event bindings (i.e., which will get unbound appropriately). 

```ts
// full component
import { BaseHTMLElement, onEvent, onDoc, onWin, onHub } from 'dnative';
...
- First you associate a class which extends `HTMLElement` with a tag name using `customElements.define('my-comp', 'MyComponent')`
  - `dnative` provides an optional convenient `@customElement('my-comp')` TypeScript decorator
...
If the component can infer its content soly from its declaration (i.e., attributes and content), then, set the `innerHTML` or call `appendChild`  in the `init()` method. Favor `this.innerHTML` or one  `this.appendChild` call (e.g., using the convenient `frag('<some-html>text</some-html>)` dnative DocumentFragment builder function)
...
```ts
import {on, off, all, first, prev, next, append, frag, attr, style } from 'dnative';
...
```ts
import {first, push, pull} from 'dnative';
...
> `dnative` formerly named [mvdom](https://github.com/mvdom/mvdom) as been rename to `dnative` as it came closer to 1.0 release. While a little longer, I felt it was punchier and more representative of the library's intent.
